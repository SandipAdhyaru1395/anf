    ...
    <div class="modal-content">
      <div class="modal-body">
        ...
        <form method="post" action="{{ route('customer.branch.store') }}" class="row g-6" id="addCustomerBranchForm">
          ...
          <div class="col-12">
            <label class="form-label" for="add_branch_contact_name">Contact Name</label>
            <input type="text" autocomplete="off" id="add_branch_contact_name" name="contact_name" value="{{ old('contact_name') ?? '' }}" class="form-control" placeholder="Contact name" />
            @error('contact_name','addBranch')
                <span class="text-danger">{{ $message }}</span>
            @enderror
          </div>
          <div class="col-12 form-control-validation">
